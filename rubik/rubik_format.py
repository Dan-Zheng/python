...
def format_string(string):
    ...
    if re.search('[^UDFBLRXYZMESudfblrxyzmes2\']', string): # fail accepted characters
        return False
    else:
        i = 0
        move_list = []
        while i < len(string):
            if re.search('[UDFBLRXYZMESudfblrxyzmes]', string[i]):
        ...
        for move in move_list:
            if re.search('[XYZ]', move): # convert [XYZ] to [xyz]
                ...
                move = move.upper()
            result = result + move + ' ' # append move to result

    result = result[0:-1]
    print 'Formatted algorithm: ' + result
    if len(move_list) >= 15:
...
def main():
    print 'This script returns the formatted string of an Rubik\'s cube algorithm (no utf-8 chars). Currently works for 2x2 to 5x5.'
    while True:
